...
const char *topicpPirani = "vacuum/p_sense/pressure";
const char *topicpIONIVAC = "vacuum/p_sense/pressureIONIVAC";
const char *topicpBlau = "vacuum/p_sense/pressureBlau";
const char *topicf = "vacuum/f_sense/frequency";
const char *topict = "vacuum/t_sense/temperature";
const char *topics = "vacuum/s_sense/Schichdicke";       
const char *topicds = "vacuum/ds_sense/DifferenzSchichtdicke";     //Aufdampfrate
...
char msg[50];
int value = 0;
float PiraniAnaloginput = 0;
float IONIVACAnaloginput = 0;
float BlauAnaloginput = 0;
float NTCinput = 0;
float frequencyNew = 100;
...
uint32_t        sample_time   = 1000000; //1000000;                                  // sample time of 1 second to count pulses
uint32_t        osc_freq      = 4000000;                                    // Oscillator frequency - initial 12543 Hz (may be 1 Hz to 40 MHz)
...
void setup()
{
...
#endif
  Serial.println("Try to init frequencyMeter");
  init_frequencyMeter ();                                                 // Initialize Frequency Meter

  //Wlan Anmeldung
  Serial.println("Try to connect WiFi");

  if (digitalRead(15) == LOW) {                                          //GPI15 == LOW Jufo WLAN 
    Serial.println("Try to connect Jufo");
    WiFi.begin(Jufossid,Jufopassword);

    while (WiFi.status() !=WL_CONNECTED) {
      delay(1000);
      Serial.println("Jufo Verbinden");
    }
    Serial.println("Jufo Verbunden");
    //MQTT Anmeldung
    client.setServer(mqtt_server_jufo, 1883);
  }

  if (digitalRead(15) == HIGH) {                                          //GPI15 == HIGH Fritzi WLAN
    Serial.println("Try to connect Fritzi");
    WiFi.begin(Fritzissid,Fritzipassword);

    while (WiFi.status() !=WL_CONNECTED) {
      delay(1000);
      Serial.println("Fritzi Verbinden");
    }
    Serial.println("Fritzi Verbunden");
    //MQTT Anmeldung
    client.setServer(mqtt_server_fritzi, 1883);
  }                                       
  ...
  pinMode(36, INPUT);                                                    //Sensor VP
  PiraniAnaloginput = analogRead(36);

  pinMode(39, INPUT);                                                    //Sensor VN
  IONIVACAnaloginput = analogRead(39);

//  pinMode(39, INPUT);                                                    //Sensor VN
//  BlauAnaloginput = analogRead(39);

  pinMode(33, INPUT);                                                    //NTC
  NTCinput = analogRead(33);
...
void loop()
{
  if (flag == true)                                                     // If count has ended
  {
    ...
    printf(" Hz \n");                                                   // Print unity Hz
    float frequencyOld = frequencyNew;
    frequencyNew = frequency;
    char ftxt[10];
    sprintf(ftxt,"%.7e",frequency);
    client.publish(topicf, ftxt);
    printf("Frequency (mqtt): %s\n", ftxt);
    
    char DS[5];                                                         //Aufdampfrate
    float Aufdampfrate = (frequencyNew-frequencyOld);
    if (Aufdampfrate > 0){
      sprintf(DS,"%.2e",Aufdampfrate);
      client.publish(topicds, DS);
      printf("Aufdampfrate / Differenz Schichtdicke (mqtt): %s\n", DS);
    }

    char S[5];                                                          //Schichtdicke
    //float SQuarz = (308*pow(10, -6)*2,65*.......)
    float Schichtdicke = (frequency-100)/1.84;
    if (Schichtdicke >= 0){
      sprintf(S,"%.2e",Schichtdicke);
      client.publish(topics, S);
      printf("Schichtdicke (mqtt): %s\n", S);
    }
    ...
    for(int i=1;i<100;i++) {                                                                     //Sensor Pirani
      PiraniAnaloginput = 0.95*PiraniAnaloginput + 0.05*analogRead(36);
      //delay(5);
    }
    printf("--------------------------------\n");
    float PiraniAnalogvoltage = (((2.5-0.19)/2900)*PiraniAnaloginput+0.167)*4;
    //analoginput = ((((analoginput-0.15)*2.56)/4095)*4)+2,5;  
    //analoginput = (((analoginput*3.3)/4095)*3);                                                // Spannungsteiler 1:3
    //printf("Piranisensor Analoginput: %f\n",PiraniAnaloginput);
    //printf("Piranisensor Analogvoltage: %f\n",PiraniAnalogvoltage);
    float p = pow(10, 1.5*PiraniAnalogvoltage-12);
    printf("Piranisensor Druck: %f\n",p);
    char ptxt[10];
    sprintf(ptxt,"%.3e",p);
    client.publish(topicpPirani, ptxt);


    for(int i=1;i<100;i++) {                                                                      //Sensor IONIVAC
      IONIVACAnaloginput = 0.95*IONIVACAnaloginput + 0.05*analogRead(39);
      //delay(5);
    }
    printf("--------------------------------\n");
    float IONIVACAnalogvoltage = (((2.5-0.19)/2900)*IONIVACAnaloginput+0.167)*4;
    if (IONIVACAnalogvoltage <= 8){
      printf("IONIVACsensor Analoginput: %f\n",IONIVACAnaloginput);
      printf("IONIVACsensor Analogvoltage: %f\n",IONIVACAnalogvoltage);
      //float pIONIVAC = 9.607792788645 * pow(10, -11) * exp(2.303511724952*IONIVACAnalogvoltage);
      float pIONIVAC = pow(10, -10+IONIVACAnalogvoltage);
      printf("IONIVACsensor Druck: %f\n",pIONIVAC);
      sprintf(ptxt,"%.3e",pIONIVAC);
      client.publish(topicpIONIVAC, ptxt);
    }


    //for(int i=1;i<100;i++) {                                                                    //Sensor Blau / Millipore
    //  BlauAnaloginput = 0.95*BlauAnaloginput + 0.05*analogRead(39);
      //delay(5);
    //}
    //float BlauAnalogvoltage = (((2.5-0.19)/2900)*BlauAnaloginput+0.167)*4;
    //analoginput = ((((analoginput-0.15)*2.56)/4095)*4)+2,5;  
    //float BlauAnalogvoltage = (((BlauAnaloginput*3.3)/4095)*3);                                 // Spannungsteiler 1:3
    //printf("Milliporesensor (Blau) Analoginput: %f\n",BlauAnaloginput);
    //printf("Milliporesensor (Blau) Analogvoltage: %f\n",BlauAnalogvoltage);
    //float pBlau = pow(10, 1.5*BlauAnalogvoltage-12);
    //sprintf(ptxt,"%.3e",BlauAnalogvoltage);
    //client.publish(topicpBlau, ptxt);
    //delay(100);                                                                                 // Delay 100 ms
    


    for(int i=1;i<100;i++) {                                                                      //NTC
      NTCinput = 0.95*NTCinput + 0.05*analogRead(33);
      //delay(5);
    }
    printf("--------------------------------\n");
    float NTCvoltage = (((2.5-0.19)/2900)*NTCinput+0.167);                                        //Umrechnung Steps in Spannung
    //printf("NTC Analoginput: %f\n",NTCinput);
    printf("NTC Analogvoltage: %f\n",NTCvoltage);
    float Rres = (NTCvoltage*2000)/(3.3-NTCvoltage);                                              //Umrechnung Spannung in Widerstand
    //printf("Rres %f\n",Rres);
    float NTCohm = (Rres*25000)/(25000-Rres); 
    printf("NTCohm: %f\n",NTCohm);                                                  
    float NTCtemp = 0.00000001823149*pow(NTCohm,3)-0.000112904828918*pow(NTCohm,2)+0.340665710941973*NTCohm-144.92934341647867;      //Umrechnung Widerstand in Temperatur       
    //printf("Temperature: %s\n", NTCtemp);
    char ttxt[10];
    sprintf(ttxt,"%.2e",NTCtemp);
    client.publish(topict, ttxt);
    printf("Temperature (mqtt): %s\n", ttxt);
    //delay(100);
