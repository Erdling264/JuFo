...
const char *topicp = "vacuum/p_sense/pressure";
...
void loop()
{
  if (flag == true)                                                     // If count has ended
  {
    ...
    for(int i=1;i<100;i++) {
      analoginput = 0.95*analoginput + 0.05*analogRead(36);
      //delay(5);
    }
    float analogvoltage = (((2.5-0.19)/2900)*analoginput+0.167)*4;
    //analoginput = ((((analoginput-0.15)*2.56)/4095)*4)+2,5;  
    //analoginput = (((analoginput*3.3)/4095)*3);                        // Spannungsteiler 1:3
    printf("Analoginput: %f\n",analoginput);
    printf("Analogvoltage: %f\n",analogvoltage);
    float p = pow(10, 1.5*analogvoltage-12);
    char ptxt[10];
    sprintf(ptxt,"%.3e",p);
    client.publish(topicp, ptxt);
